...
A fast, read-only **terminal dashboard for your DigitalOcean Droplets**. List every Droplet with live CPU / memory / disk usage, search and sort, expand any Droplet into full time-series charts, and SSH straight into one — all without leaving the terminal.
...
Sailor is **read-only by design** — it observes and connects, but never powers off, reboots, resizes, or destroys Droplets. That keeps it safe to run against a production account.
